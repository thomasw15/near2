...
def run_accuracy_experiment(case=20):
    ...
    for i in range(case):
        ...
        Xinf, inf_list_temp = solve_infprod(A, B, C, F, G, H, X_true, rho=0.01)
        X1, one_list_temp = solve_1prod(A, B, C, F, G, H, X_true, rho=0.01)
        
        Xcpinf, cp_timeinf = cp_prod_inf(A, B, C, F, G, H, e=1e-16, a=10)
        Xcp1, cp_time1 = cp_prod_1(A, B, C, F, G, H, e=1e-16, a=10)
    ...
    # Save data
    np.savez('accuracy_data_prod.npz',
