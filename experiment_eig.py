...
def eig_Frobenius(A, B, C, UB, sB, VBT, rB, UC, sC, VCT, rC, eigenvalue):
    n = A.shape[0]
    Xhat = rank_Frobenius(A - eigenvalue * B @ C, B, C, UB, sB, VBT, rB, UC, sC, VCT, rC, n-1)
    I = np.eye(Xhat.shape[0])
    #X = Xhat + eigenvalue * np.eye(n)
    X = Xhat + eigenvalue * I
    return X
...
def solve_1eig(A, B, C, X_true, eigenvalue, rho, iteration = 300):
    UB, sB, VBT = la.svd(B)
    rB = la.matrix_rank(B)
    UC, sC, VCT = la.svd(C)
    rC = la.matrix_rank(C)
    ...
    for i in range(iteration):
        ...
        # X-update
        X = eig_Frobenius(A + D - Y, B, C, UB, sB, VBT, rB, UC, sC, VCT, rC, eigenvalue)
        D = D - Y + A - B @ X @ C
        forward_error = la.norm(X - X_true, "fro")/la.norm(X_true, "fro")
        forward_error_list.append(forward_error)
    return X, forward_error_list
        
def solve_infeig(A, B, C, X_true, eigenvalue, rho, iteration = 300):
    UB, sB, VBT = la.svd(B)
    rB = la.matrix_rank(B)
    UC, sC, VCT = la.svd(C)
    rC = la.matrix_rank(C)
...
def solve_3eig(A, B, C, X_true, eigenvalue, rho, iteration = 300):
    UB, sB, VBT = la.svd(B)
    rB = la.matrix_rank(B)
    UC, sC, VCT = la.svd(C)
    rC = la.matrix_rank(C)
...
def tol_1eig(A, B, C, X_true, eigenvalue, rho, tol=1e-8):
    UB, sB, VBT = la.svd(B)
    rB = la.matrix_rank(B)
    UC, sC, VCT = la.svd(C)
    rC = la.matrix_rank(C)
...
def tol_infeig(A, B, C, X_true, eigenvalue, rho, tol=1e-8):
    UB, sB, VBT = la.svd(B)
    rB = la.matrix_rank(B)
    UC, sC, VCT = la.svd(C)
    rC = la.matrix_rank(C)
...
def tol_3eig(A, B, C, X_true, eigenvalue, rho, tol=1e-8):
    UB, sB, VBT = la.svd(B)
    rB = la.matrix_rank(B)
    UC, sC, VCT = la.svd(C)
    rC = la.matrix_rank(C)
...
def run_accuracy_experiment(case=20):
    ...
    for i in range(case):
        ...
        X_true = rank_r_approximation(X_true - eigenvalue * np.eye(n),n-1) + eigenvalue * np.eye(n)
        A = B @ X_true @ C
        
        Xinf, inf_list_temp = solve_infeig(A, B, C, X_true, eigenvalue, rho=0.01)
        X1, one_list_temp = solve_1eig(A, B, C, X_true, eigenvalue, rho=0.01)
        X3, three_list_temp = solve_3eig(A, B, C, X_true, eigenvalue, rho=0.01)
        inf_list.append(inf_list_temp[299])
        one_list.append(one_list_temp[299])
        three_list.append(three_list_temp[299])
    # Save data
    np.savez('accuracy_data_eig.npz',
...
def plot_accuracy_results_nuc(data_file='accuracy_data_eig.npz'):
    ...
    plt.yscale('log')
    plt.legend()
    plt.savefig('eig_acc_eig.pdf', dpi=300, bbox_inches='tight')
    plt.close()

def plot_accuracy_results_inf(data_file='accuracy_data_eig.npz'):
    """Plot accuracy results from saved data"""
    data = np.load(data_file)
    n_list = data['n_list']
    ...
    plt.yscale('log')
    plt.legend()
    plt.savefig('eig_acc_inf.pdf', dpi=300, bbox_inches='tight')
    plt.close()

def plot_accuracy_results_3(data_file='accuracy_data_eig.npz'):
    """Plot accuracy results from saved data"""
    data = np.load(data_file)
    n_list = data['n_list']
    ...
    plt.yscale('log')
    plt.legend()
    plt.savefig('eig_acc_3.pdf', dpi=300, bbox_inches='tight')
    plt.close()
...
def run_speed_experiment():
    ...
    for n in dimensions_ours:
        ...
        for _ in range(10):
            ...
            _ = tol_1eig(A, B, C, X_true, eigenvalue, rho=0.0000001, tol=1e-8)
            nuclear_temp += timeit.default_timer() - start
            
            # Spectral norm timing
            start = timeit.default_timer()
            _ = tol_infeig(A, B, C, X_true, eigenvalue, rho=0.0000001, tol=1e-8)
            spectral_temp += timeit.default_timer() - start

            # Schatten 3-norm timing
            start = timeit.default_timer()
            _ = tol_3eig(A, B, C, X_true, eigenvalue, rho=0.0000001, tol=1e-8)
...
def plot_speed_results(data_file='speed_data_eig.npz'):
    ...
    plt.yscale('log')
    plt.legend()
    plt.savefig('speed_comparison_eig.pdf', dpi=300, bbox_inches='tight')
    plt.close()
